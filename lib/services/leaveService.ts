...
export class StaffUpdateConflictError extends Error {}
...
export async function updateStaffRoles(
  ...
  if (roleNames.length === 0) {
    throw new Error("พนักงานต้องมีบทบาทอย่างน้อย 1 บทบาท");
  }
  if (roleNames.length > 1) {
    throw new Error("พนักงาน 1 คนสามารถมีได้เพียง 1 บทบาท");
  }
...
export async function createRole(roleName: string) {
  ...
  if (!trimmed) throw new Error("กรุณากรอกชื่อ Role");
  const existing = await prisma.role.findFirst({
    where: { role_name: { equals: trimmed, mode: "insensitive" } },
  });
  if (existing) throw new Error("มี Role นี้อยู่ในระบบแล้ว");
...
export async function updateRoleName(roleId: string, roleName: string) {
  ...
  if (!role) throw new Error("ไม่พบ Role ในระบบ");
  if (SYSTEM_ROLE_NAMES.includes(role.role_name.toUpperCase())) {
    throw new Error("ไม่สามารถแก้ไข Role ระบบได้");
  }
  const trimmed = roleName.trim();
  if (!trimmed) throw new Error("กรุณากรอกชื่อ Role");
  ...
  if (duplicate) throw new Error("มี Role นี้อยู่ในระบบแล้ว");
...
export async function toggleRoleActive(roleId: string, isActive: boolean) {
  ...
  if (SYSTEM_ROLE_NAMES.includes(role.role_name.toUpperCase()) && !isActive) {
    throw new Error("ไม่สามารถปิดใช้งาน Role ระบบได้");
  }
  if (!isActive && role._count.staffRoles > 0) {
    throw new Error("ไม่สามารถปิดใช้งาน Role ที่มีพนักงานใช้งานอยู่ได้");
  }
  const updated = await prisma.role.update({
    where: { role_id: roleId },
