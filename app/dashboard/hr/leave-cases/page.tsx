...
export default function LeaveCasesPage() {
  const router = useRouter();
  const [fetchKey, setFetchKey] = useState(0);

  const [searchTerm, setSearchTerm] = useState("");
  const [data, setData] = useState<LeaveCaseListItem[]>([]);
  ...
  const filtered = data.filter(
    (c) =>
      ...
      c.leaveTypeName.toLowerCase().includes(searchTerm.toLowerCase()),
  );
  ...
  const handleToggleActive = async (id: string, currentActive: boolean) => {
    ...
    try {
      ...
      toast.success(!currentActive ? "เปิดใช้งานกรณีการลาเรียบร้อย" : "ปิดใช้งานกรณีการลาเรียบร้อย");
    } catch (err) {
      ...
      <DashboardContent>
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-md mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button
              onClick={() => router.push("/dashboard/hr/leave-cases/add")}
        ...
        <div className="bg-white rounded-xl border border-[#c8c5d0] shadow-lg overflow-hidden">
          <div className="px-6 py-4 border-b border-[#c8c5d0] flex items-center justify-between bg-slate-50/50">
            ...
            <span className="text-[13px] leading-[18px] text-[#47464f] italic">Showing {filtered.length} leave cases</span>
          </div>
              ...
              <Table containerClassName="max-h-[500px] overflow-y-auto">
                <TableHeader className="sticky top-0 z-10 bg-[#1e1b4b]">
                  <TableRow className="hover:bg-transparent border-none">
                    <TableHead className="text-white font-semibold px-6 py-4 text-[13px] leading-[16px] tracking-[0.02em] uppercase">ชื่อกรณี</TableHead>
                    <TableHead className="text-white font-semibold px-6 py-4 text-[13px] leading-[16px] tracking-[0.02em] uppercase">ประเภทการลา</TableHead>
                ...
                <TableBody className="divide-y divide-[#c8c5d0]">
                  {filtered.length > 0 ? (
                    filtered.map((c) => (
                      <TableRow key={c.leaveCaseId} className="hover:bg-[#eff4ff]/30 transition-all duration-200">
                        <TableCell className="px-6 py-4 text-[14px] leading-[20px] font-semibold whitespace-nowrap">{c.caseName}</TableCell>
                        <TableCell className="px-6 py-4 text-[14px] leading-[20px] whitespace-nowrap">
                          <span className="bg-slate-100 px-3 py-1 rounded-full text-slate-600 font-medium text-[13px]">{c.leaveTypeName}</span>
                        </TableCell>
                        <TableCell className="px-6 py-4 text-[14px] leading-[20px] text-center whitespace-nowrap">
                          <span className="bg-slate-100 px-3 py-1 rounded-full text-slate-600 font-medium text-[13px]">{c.leaveCount}</span>
                        </TableCell>
                        <TableCell className="px-6 py-4 whitespace-nowrap">
                          <span className={`inline-flex items-center gap-1 px-4 py-1 rounded-full text-[12px] leading-[16px] font-semibold tracking-[0.05em] whitespace-nowrap border ${c.isActive ? "bg-emerald-50 text-emerald-700 border-emerald-100" : "bg-gray-100 text-black border-gray-300"}`}>
                            <span className={`w-1.5 h-1.5 rounded-full ${c.isActive ? "bg-emerald-500" : "bg-gray-400"}`}></span>
                    ...
                    <TableRow>
                      <TableCell colSpan={5} className="text-center py-20 text-[#47464f]">
                        <div className="flex flex-col items-center gap-2">
                          ...
                          <span>ไม่พบข้อมูลกรณีการลา</span>
                        </div>
                      </TableCell>
                    </TableRow>
                  )}
