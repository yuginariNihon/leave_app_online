...
import type { EmploymentTypeListItem } from "@/lib/services/leaveService";
...
export default function EmployeeTypesPage() {
  ...
  const [data, setData] = useState<EmploymentTypeListItem[]>([]);
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      setError("");
      try {
        const json = await apiFetch<{ data: EmploymentTypeListItem[] }>("/api/hr/employee-types");
        if (!cancelled) setData(json.data);
      } catch {
        if (!cancelled) setError("ไม่สามารถโหลดข้อมูลประเภทพนักงานได้");
      } finally {
  ...
  const filtered = data.filter(
    (et) =>
      ...
      (et.thainame?.toLowerCase() ?? "").includes(searchTerm.toLowerCase()) ||
  ...
  const handleToggleActive = async (id: string, currentActive: boolean) => {
    ...
    try {
      await apiFetch(`/api/hr/employee-types/${id}`, {
        method: "PATCH",
        body: JSON.stringify({ isActive: !currentActive }),
      });
      setData((prev) =>
        prev.map((et) => (et.employmentTypeId === id ? { ...et, isActive: !currentActive } : et)),
      );
      toast.success(!currentActive ? "เปิดใช้งานประเภทพนักงานเรียบร้อย" : "ปิดใช้งานประเภทพนักงานเรียบร้อย");
    } catch (err) {
  ...
  return (
        ...
        <AppBreadcrumb
          items={[{ label: "Home", href: "/dashboard" }, { label: "HR" }, { label: "จัดการประเภทพนักงาน" }]}
          className="mb-4"
        />

        <div className="flex flex-col md:flex-row md:items-end justify-between gap-md mb-6">
          <div>
            <h1 className="text-[32px] font-bold leading-[40px] tracking-[-0.02em] text-[#070235]">จัดการประเภทพนักงาน</h1>
            <p className="text-[14px] leading-[20px] text-[#47464f]">Manage organizational classifications and employment categories.</p>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Button
              onClick={() => router.push("/dashboard/hr/employee-types/add")}
        ...
        <div className="bg-white rounded-xl border border-[#c8c5d0] shadow-lg overflow-hidden">
          <div className="px-6 py-4 border-b border-[#c8c5d0] flex items-center justify-between bg-slate-50/50">
            ...
            <span className="text-[13px] leading-[18px] text-[#47464f] italic">Showing {filtered.length} categories</span>
          </div>
              ...
              <Table containerClassName="max-h-[500px] overflow-y-auto">
                <TableHeader className="sticky top-0 z-10 bg-[#1e1b4b]">
                  <TableRow className="hover:bg-transparent border-none">
                    ...
                    <TableHead className="text-white font-semibold px-6 py-4 text-[13px] leading-[16px] tracking-[0.02em] uppercase">ชื่อ (Name EN)</TableHead>
                    <TableHead className="text-white font-semibold px-6 py-4 text-[13px] leading-[16px] tracking-[0.02em] uppercase">ชื่อภาษาไทย (Name TH)</TableHead>
                ...
                <TableBody className="divide-y divide-[#c8c5d0]">
                  {filtered.length > 0 ? (
                    filtered.map((et) => (
                      <TableRow key={et.employmentTypeId} className="hover:bg-[#eff4ff]/30 transition-all duration-200">
                        <TableCell className="px-6 py-4 text-[14px] leading-[20px] font-semibold text-[#070235] whitespace-nowrap">{et.code}</TableCell>
                        <TableCell className="px-6 py-4 text-[14px] leading-[20px] font-medium whitespace-nowrap">{et.name}</TableCell>
                        <TableCell className="px-6 py-4 text-[14px] leading-[20px] whitespace-nowrap">{et.thainame ?? "-"}</TableCell>
                        <TableCell className="px-6 py-4 text-[14px] leading-[20px] text-center whitespace-nowrap">
                          <span className="bg-slate-100 px-3 py-1 rounded-full text-slate-600 font-medium text-[13px]">{et.staffCount}</span>
                        </TableCell>
                        <TableCell className="px-6 py-4">
                          <span className={`inline-flex items-center gap-1 px-4 py-1 rounded-full text-[12px] leading-[16px] font-semibold tracking-[0.05em] whitespace-nowrap border ${et.isActive ? "bg-emerald-50 text-emerald-700 border-emerald-100" : "bg-gray-100 text-black border-gray-300"}`}>
                            <span className={`w-1.5 h-1.5 rounded-full ${et.isActive ? "bg-emerald-500" : "bg-gray-400"}`}></span>
                    ...
                    <TableRow>
                      <TableCell colSpan={6} className="text-center py-20 text-[#47464f]">
                        <div className="flex flex-col items-center gap-2">
                          ...
                          <span>ไม่พบข้อมูลประเภทพนักงาน</span>
                        </div>
                      </TableCell>
                    </TableRow>
                  )}
