...
export async function GET(request: NextRequest) {
  try {
    ...
    logReadAccess(session.userId, session.staffId, "approval-history", undefined, headersList.get("x-forwarded-for")?.split(",")[0].trim(), headersList.get("user-agent") ?? undefined);
